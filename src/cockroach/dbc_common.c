...
{
	char buf[1024];
	if(strcmp(pghost, "localhost") == 0 && strcmp(pgport, "26257") == 0) {
		sprintf(buf, "application_name=dbt2-client dbname=%s", dbname);
	} else {
		sprintf(buf, "application_name=dbt2-client host=%s port=%s dbname=%s",
				pghost, pgport, dbname);
	}
	dbc->conn = PQconnectdb(buf);
	if (PQstatus(dbc->conn) != CONNECTION_OK) {
		LOG_ERROR_MESSAGE("Connection to database '%s' failed.", dbname);
		LOG_ERROR_MESSAGE("%s", PQerrorMessage(dbc->conn));
		PQfinish(dbc->conn);
		return ERROR;
	}
