...
int _connect_to_db(struct db_context_t *odbcc)
{
	...
	rc = SQLConnect(odbcc->library.odbc.hdbc, servername, SQL_NTS,
	...
	rc = SQLSetConnectAttr(odbcc->library.odbc.hdbc, SQL_ATTR_TXN_ISOLATION,
	...
	rc = SQLAllocHandle(SQL_HANDLE_STMT, odbcc->library.odbc.hdbc, &odbcc->library.odbc.hstmt);
	if (rc != SQL_SUCCESS && rc != SQL_SUCCESS_WITH_INFO) {
		LOG_ODBC_ERROR(SQL_HANDLE_STMT, odbcc->library.odbc.hstmt);
		return ERROR;
	}
		...
		struct sql_result_t * sql_result, char * query_name)
{
	...
	rc = SQLExecDirect(dbc->library.odbc.hstmt, (unsigned char *) query, SQL_NTS);
	if (rc != SQL_SUCCESS && rc != SQL_SUCCESS_WITH_INFO) {
		LOG_ODBC_ERROR(SQL_HANDLE_STMT, dbc->library.odbc.hstmt);
		return 0;
	}
	rc = SQLNumResultCols(dbc->library.odbc.hstmt,&sql_result->library.odbc.num_fields);
	if (rc != SQL_SUCCESS && rc != SQL_SUCCESS_WITH_INFO) {
		LOG_ODBC_ERROR(SQL_HANDLE_STMT, dbc->library.odbc.hstmt);
		return 0;
	}

	rc = SQLRowCount(dbc->library.odbc.hstmt, (long int *) &sql_result->library.odbc.num_rows);
	if (rc != SQL_SUCCESS && rc != SQL_SUCCESS_WITH_INFO)  {
		LOG_ODBC_ERROR(SQL_HANDLE_STMT, dbc->library.odbc.hstmt);
		return 0;
	}
	...
	if (sql_result->library.odbc.num_fields) {
		sql_result->library.odbc.lengths= malloc(sizeof(int) * sql_result->library.odbc.num_fields);

		for (i=0; i < sql_result->library.odbc.num_fields; i++) {
			SQLDescribeCol(dbc->library.odbc.hstmt,
					(SQLSMALLINT)(i + 1),
					colname,
					sizeof(colname),
					&colnamelen,
		...
		struct sql_result_t *sql_result, int field)
{
	...
	if (sql_result->library.odbc.current_row && field < sql_result->library.odbc.num_fields) {
		if ((tmp = calloc(sizeof(char), sql_result->library.odbc.lengths[field] + 1))) {
			rc = SQLGetData(dbc->library.odbc.hstmt, field + 1, SQL_C_CHAR, tmp,
					sql_result->library.odbc.lengths[field] + 1, (long int *) &cb_var);
			if (rc != SQL_SUCCESS && rc != SQL_SUCCESS_WITH_INFO) {
				LOG_ODBC_ERROR(SQL_HANDLE_STMT, dbc->library.odbc.hstmt);
			}
		} else {
