...
int delivery_nonsp(
		struct db_context_t *dbc, struct delivery_t *data, char **vals,
		int nvals) {
	/* Input variables. */
	int w_id = data->w_id;
	...
	for (d_id = 1; d_id <= 10; d_id++) {
...
#endif
		if ((*dbc->sql_execute)(dbc, query, &result, "DELIVERY_1") &&
			result.library.sqlite.query_running) {
			(*dbc->sql_fetchrow)(dbc, &result);
			vals[NO_O_ID] = (char *) (*dbc->sql_getvalue)(
					dbc, &result,
					0); // NO_O_ID
			...
			if (!vals[NO_O_ID]) {
				LOG_ERROR_MESSAGE(
						"ERROR: NO_O_ID=NULL for query DELIVERY_1:\n%s\n",
						query);
				// return -1;
			}
		} else {
		...
		if (vals[NO_O_ID] && atoi(vals[NO_O_ID]) > 0) {
...
#endif
			if ((*dbc->sql_execute)(dbc, query, &result, "DELIVERY_3") &&
				result.library.sqlite.query_running) {
				(*dbc->sql_fetchrow)(dbc, &result);
				vals[O_C_ID] = (char *) (*dbc->sql_getvalue)(
						dbc, &result,
						0); // O_C_ID
				(*dbc->sql_close_cursor)(dbc, &result);
...
#endif
			if ((*dbc->sql_execute)(dbc, query, &result, "DELIVERY_6") &&
				result.library.sqlite.query_running) {
				(*dbc->sql_fetchrow)(dbc, &result);
				vals[OL_AMOUNT] = (char *) (*dbc->sql_getvalue)(
						dbc, &result,
						0); // OL_AMOUNT
				(*dbc->sql_close_cursor)(dbc, &result);
			...
			snprintf(
					query, 250, DELIVERY_7, vals[OL_AMOUNT], vals[O_C_ID], w_id,
					d_id);

#ifdef DEBUG_QUERY
			LOG_ERROR_MESSAGE(
					"DELIVERY_7: query %s LEN %d\n", query, strlen(query));
#endif
			if (!(*dbc->sql_execute)(dbc, query, &result, "DELIVERY_7")) {
				LOG_ERROR_MESSAGE(
						"DELIVERY_7: OL_AMOUNT: |%s| O_C_ID: |%s| query %s",
						vals[OL_AMOUNT], vals[O_C_ID], query);
				return -1;
			}
		}
		dbt2_free_values(vals, nvals);
