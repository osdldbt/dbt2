...
SQL="SELECT SUM(ol_amount) FROM dbt.order_line WHERE ol_w_id = 1 AND ol_d_id = 1 AND ol_o_id = 1"
echo $SQL
_o=`cat <<EOF | /opt/sapdb/depend/bin/dbmcli -d $DBNAME -u dba,dba -uSQL dbt,dbt 2>&1
sql_execute explain $SQL
sql_execute select * from show
...
SQL="SELECT c_first, c_middle, c_last, c_street_1, c_street_2, c_city, c_state, c_zip, c_phone, CHAR(c_since, ISO), c_credit, c_credit_lim, c_discount, c_balance, c_data, c_id, c_ytd_payment FROM dbt.customer WHERE c_last = 'a' ORDER BY c_first ASC"
echo $SQL
_o=`cat <<EOF | /opt/sapdb/depend/bin/dbmcli -d $DBNAME -u dba,dba -uSQL dbt,dbt 2>&1
sql_execute explain $SQL
sql_execute select * from show
...
SQL="SELECT d_next_o_id FROM dbt.district WHERE d_w_id = 1 AND d_id = 1"
echo $SQL
_o=`cat <<EOF | /opt/sapdb/depend/bin/dbmcli -d $DBNAME -u dba,dba -uSQL dbt,dbt 2>&1
sql_execute explain $SQL
sql_execute select * from show
