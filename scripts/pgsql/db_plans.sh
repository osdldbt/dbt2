...
if [ "$OUTPUT_FILE" == "" ]; then
	echo "usage: db_plan.sh -o <outfile>"
	exit 1
fi
...
SQL="EXPLAIN UPDATE customer SET c_balance = c_balance - 1.0, c_ytd_payment = c_ytd_payment + 1, c_data = 'hello dogger' WHERE c_id = 1 AND c_w_id
 = 1 AND c_d_id = 1;"
echo "$SQL" >> $OUTPUT_FILE
$PSQL -d dbt2 -c "$SQL" >> $OUTPUT_FILE
